...
!function (window) {
    ...
    function TopSheet (element, closeElement) {
        this.$element = $(element);
        this.closeElement = closeElement;
        this.toggleClass = 'topsheet-toggle';
    }

    TopSheet.prototype.open = function () {
        ...
        // 第三方关闭窗口操作
        if (_this.closeElement) {
            $doc.on('click.ydui.topsheet', _this.closeElement, function () {
                _this.close();
            });
        }

        _this.$element.addClass(_this.toggleClass).trigger('open.ydui.topsheet');
    };

    TopSheet.prototype.close = function () {
        var _this = this;

        YDUI.device.isIOS && $('.g-scrollview').removeClass('g-fix-ios-overflow-scrolling-bug');

        $mask.off('click.ydui.topsheet.mask').remove();
        _this.$element.removeClass(_this.toggleClass).trigger('close.ydui.topsheet');
        //$doc.off('click.ydui.topsheet', _this.closeElement);
    };
    ...
    function Plugin (option) {
        ...
        return this.each(function () {
            var $this = $(this),
                topsheet = $this.data('ydui.topsheet');

            if (!topsheet) {
                $this.data('ydui.topsheet', (topsheet = new TopSheet(this, option.closeElement)));
                if (!option || typeof option == 'object') {
                    topsheet.open();
                }
            }
            ...
            if (typeof option == 'string') {
                topsheet[option] && topsheet[option].apply(topsheet, args);
            }
        });
    }

    $doc.on('click.ydui.topsheet.data-api', '[data-ydui-topsheet]', function (e) {
        e.preventDefault();

        var options = window.YDUI.util.parseOptions($(this).data('ydui-topsheet')),
            $target = $(options.target),
            option = $target.data('ydui.topsheet') ? 'open' : options;
